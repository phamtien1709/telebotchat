...
class CommonController {
  async getPrice(message) {
    ...
    let newMessage = ''
    newMessage += `Cap nhat gia coin dang quan tam(24h) (${dayjs().format('YYYY-MM-DD HH:mm:ss')}):\n`
    for (let i = 0; i < interestedResponse.length; i++) {
      newMessage += `\xF0\x9F\x92\xB0 ${interestedResponse[i].symbol}: \xF0\x9F\x92\xB2${interestedResponse[
        i
      ].lastPrice.slice(0, -3)}. ${
        interestedResponse[i].priceChangePercent.includes('-')
          ? '\xF0\x9F\x93\x89'
          : '\xF0\x9F\x93\x88'
      }: ${interestedResponse[i].priceChangePercent.slice(0, -1)}%\n`
    }
